...
export default function Reparaciones() {
  ...
  async function getReparaciones(){
    await API.get('/api.reportesolicitud/solicitud/1')
      .then(
      res => {
  ...
  async function updateReparaciones(newData, oldData){
    ...
    return (
        <MaterialTable
          ...
          localization = {{
            ...
            toolbar:{
                searchPlaceholder: 'Buscar Reparacion'
            }
          }}
          columns = {[
          ...
          data={reparaciones} //Aqui es donde se cargan los datos del Get en la tabla          
          detailPanel={[
            {
              tooltip: 'Detalles',
              render: data => {
            ...
            onRowUpdate: (newData, oldData) =>
              {
                return new Promise(resolve => {
                  setTimeout(() => {
                    resolve();
                    console.log(oldData);
                    console.log("Solicitud de Reparación: " + newData);
                    updateReparaciones(newData, oldData);
                  }, 600);
                });
              },
            }}
